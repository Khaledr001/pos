...
`;function J(e,t=25){const n=i(),a=e.trim();if(!a)return n.prepare(`SELECT ${S} FROM variants v ${L}
    ...
    FROM customers`;if(!a)return n.prepare(`${r} ORDER BY name LIMIT ?`).all(t);const s=`%${a}%`;return n.prepare(`${r} WHERE name LIKE ? OR company LIKE ? OR phone LIKE ? ORDER BY name LIMIT ?`).all(s,s,s,t)}function b(){const e=i(),t=e.prepare(`SELECT client_id AS id, payload
       ...
       FROM local_sales WHERE cash_session_id = ? AND status = 'completed'`).get(t);return{cashIn:String(a),cashOut:String(r),cashSales:String(s.total??0)}}function Z(e,t){const n=i(),a=b();if(a)return a;const r=w.randomUUID(),s=new Date().toISOString();return g(n,{clientId:r,entity:"cash_session",occurredAt:s,payload:{branchId:t,openingAmount:e,openedAt:s}}),{id:r,openingAmount:e,openedAt:s,status:"open",cashIn:"0",cashOut:"0",cashSales:"0"}}function ee(e,t){const n=i(),a=n.prepare(`SELECT client_id AS id, payload FROM outbox
       WHERE entity = 'cash_session' ORDER BY sequence DESC LIMIT 1`).get();if(!a)return;const r=JSON.parse(a.payload);r.closedAt=new Date().toISOString(),r.countedAmount=e,t&&(r.notes=t),n.prepare("UPDATE outbox SET payload = ?, status = 'pending' WHERE client_id = ?").run(JSON.stringify(r),a.id)}function te(e,t,n){const a=i(),r=b();if(!r)throw new Error("No drawer is open on this terminal");g(a,{clientId:w.randomUUID(),entity:"cash_movement",occurredAt:new Date().toISOString(),payload:{cashSessionId:r.id,type:e,amount:t,reason:n}})}function ne(e){const t=i(),n=e.payments.reduce((a,r)=>a+Number(r.amount),0);return t.transaction(()=>{t.prepare(`INSERT INTO local_sales
       ...
       FROM local_sale_items WHERE sale_client_id = ? ORDER BY sort_order`).all(t)}function se(e){return e.prepare("SELECT COALESCE(MAX(sequence), 0) + 1 AS next FROM outbox").get().next}function g(e,t){e.prepare(`INSERT INTO outbox (client_id, entity, sequence, occurred_at, payload)
     VALUES (?, ?, ?, ?, ?)
     ON CONFLICT(client_id) DO NOTHING`).run(t.clientId,t.entity,se(e),t.occurredAt,JSON.stringify(t.payload))}function ie(e=200){return i().prepare(`SELECT client_id AS clientId, entity, sequence, occurred_at AS occurredAt, payload
       FROM outbox WHERE status = 'pending' ORDER BY sequence LIMIT ?`).all(e).map(a=>({clientId:a.clientId,entity:a.entity,sequence:a.sequence,occurredAt:a.occurredAt,payload:JSON.parse(a.payload)}))}function oe(){const t=i().prepare(`SELECT
         SUM(CASE WHEN status = 'pending'  THEN 1 ELSE 0 END) AS pending,
         SUM(CASE WHEN status = 'rejected' THEN 1 ELSE 0 END) AS failed
       FROM outbox`).get();return{pending:t.pending??0,failed:t.failed??0}}function ce(e){const t=i();if(e.outcome==="applied"||e.outcome==="duplicate"){t.transaction(()=>{t.prepare(`UPDATE outbox SET status = 'synced', server_id = ?, document_number = ?, last_error = NULL
       ...
       WHERE client_id = ?`).run(e.message??"Rejected by the server",e.clientId);return}t.prepare("UPDATE outbox SET attempts = attempts + 1, last_error = ? WHERE client_id = ?").run(e.message??null,e.clientId)}function de(){i().prepare(`UPDATE inventory SET local_delta = '0'
     WHERE variant_id IN (
       ...
       ON CONFLICT(key) DO UPDATE SET value = excluded.value`).run(e,t)}class U extends Error{constructor(t,n,a){super(a),this.status=t,this.code=n,this.name="ApiError"}status;code;get isPermanent(){return this.status>=400&&this.status<500&&this.status!==408&&this.status!==429}}let E=null,y=null;function B(){return u("api_url")}function O(){return u("device_id")}function le(){return u("branch_id")}function ue(){return E!==null||u("refresh_token")!==null}function Te(){E=null,N("refresh_token",null)}async function Ee(e){const t=X(),n=O(),a=le();if(!n||!a)throw new Error("This terminal has not been activated yet");const r=await k(`${t}/auth/pin-login`,{method:"POST",body:JSON.stringify({pin:e,deviceId:n,branchId:a})});return M(r.accessToken,r.refreshToken,r.expiresIn),r.user}function M(e,t,n){E={accessToken:e,refreshToken:t,expiresAt:Date.now()+Math.max(0,n-60)*1e3},N("refresh_token",t)}async function _e(){if(E&&Date.now()<E.expiresAt)return E.accessToken;if(y??=pe().finally(()=>{y=null}),await y,!E)throw new Error("This terminal is signed out. Sign in with a PIN.");return E.accessToken}async function pe(){const e=E?.refreshToken??u("refresh_token");if(!e)throw new Error("This terminal is signed out. Sign in with a PIN.");try{const t=await k(`${X()}/auth/refresh`,{method:"POST",body:JSON.stringify({refreshToken:e})});M(t.accessToken,t.refreshToken,t.expiresIn)}catch(t){throw t instanceof U&&t.isPermanent&&Te(),t}}async function G(e,t){const n=await _e();return k(`${X()}${e}`,{method:"POST",headers:{authorization:`Bearer ${n}`},body:JSON.stringify(t)})}async function Ne(){const e=B();if(!e)return!1;try{const t=new AbortController,n=setTimeout(()=>t.abort(),4e3),a=await fetch(new URL("/health",e),{signal:t.signal});return clearTimeout(n),a.ok}catch{return!1}}function X(){const e=B();if(!e)throw new Error("No server address is configured on this terminal");return e.replace(/\/+$/,"")}async function k(e,t){const n=new AbortController,a=setTimeout(()=>n.abort(),3e4);let r;try{r=await fetch(e,{...t,signal:n.signal,headers:{"content-type":"application/json",...t.headers??{}}})}finally{clearTimeout(a)}const s=await r.text(),l=s?JSON.parse(s):{};if(!r.ok||l.success===!1){const _=l.error??{};throw new U(r.status,_.code??"UNKNOWN",_.message??`Request failed with ${r.status}`)}return l.data??l}const me=500;let I=null,m=null,H=()=>null;const p={online:!1,lastPullAt:null,lastPushAt:null,lastCheckpoint:null,pendingPushCount:0,failedPushCount:0,syncing:!1,lastError:null};function T(e={}){Object.assign(p,e);const t=oe();return p.pendingPushCount=t.pending,p.failedPushCount=t.failed,H()?.webContents.send("sync:status-changed",{...p}),{...p}}function he(e,t){H=t,p.lastCheckpoint=u("checkpoint"),e.handle("sync:status",()=>T()),e.handle("sync:now",()=>R());const n=Number(process.env.POS_SYNC_INTERVAL_MS??3e4);I=setInterval(()=>{R().catch(()=>{})},n)}function Se(){I&&(clearInterval(I),I=null)}async function R(){return m||(m=(async()=>{if(!O())return T({online:!1,lastError:"This terminal has not been activated"});if(T({syncing:!0,lastError:null}),!await Ne())return T({syncing:!1,online:!1});if(!ue())return T({syncing:!1,online:!0,lastError:"Signed out — enter a PIN"});try{return await Le(),await Ie(),de(),T({syncing:!1,online:!0,lastError:null})}catch(t){const n=t instanceof Error?t.message:"Sync failed";return T({syncing:!1,online:t instanceof U,lastError:n})}})().finally(()=>{m=null}),m)}async function Le(){const e=ie(200);if(e.length===0)return;const t=await G("/sync/push",{deviceId:O(),lastCheckpoint:u("checkpoint"),items:e});for(const n of t.results)ce(n);T({lastPushAt:new Date().toISOString()})}async function Ie(){for(let e=0;e<200;e+=1){const t=await G("/sync/pull",{deviceId:O(),since:u("checkpoint"),limit:me});if(Ae(t.changes,t.checkpoint),T({lastPullAt:new Date().toISOString(),lastCheckpoint:t.checkpoint}),!t.hasMore)return}}function Ae(e,t){const n=i();n.transaction(()=>{for(const a of e){if(a.deleted){Oe(a.entity,a.id);continue}a.record&&fe(a.entity,a.id,a.record)}n.prepare(`INSERT INTO device_state (key, value) VALUES ('checkpoint', ?)
       ON CONFLICT(key) DO UPDATE SET value = excluded.value`).run(t)})()}function Oe(e,t){const n=i(),a={product:"variants",customer:"customers",category:null,unit:null}[e];a&&n.prepare(`DELETE FROM ${a} WHERE id = ?`).run(t),n.prepare(`INSERT INTO deleted_records (entity, id, deleted_at) VALUES (?, ?, datetime('now'))
     ON CONFLICT(entity, id) DO NOTHING`).run(e,t)}function fe(e,t,n){const a=i(),r=s=>s==null?null:String(s);switch(e){case"product":a.prepare(`INSERT INTO variants
         ...
         ON CONFLICT(id) DO UPDATE SET
           ...
           credit_on_hold = excluded.credit_on_hold, updated_at = datetime('now')`).run(t,r(n.name),r(n.company),r(n.phone),r(n.trn),r(n.priceListId),r(n.creditLimit)??"0",r(n.creditBalance)??"0",n.creditOnHold?1:0);return;default:return}}function h(){R().catch(()=>{})}function ve(e){e.handle("catalog:search",(t,n,a)=>J(n??"",a)),e.handle("catalog:by-barcode",(t,n)=>P(n??"")),e.handle("customers:search",(t,n)=>z(n??"")),e.handle("cash:current",()=>b()),e.handle("cash:open",(t,n)=>Z(String(n??"0"),u("branch_id"))),e.handle("cash:close",(t,n,a)=>{ee(String(n??"0"),a),h()}),e.handle("cash:movement",(t,n,a,r)=>{te(n,String(a??"0"),r??"")}),e.handle("sales:commit",(t,n)=>{const a=ne(n);return h(),a}),e.handle("sales:recent",(t,n)=>ae(n)),e.handle("sales:find",(t,n)=>re(n??"")),e.handle("auth:pin-login",async(t,n)=>{const a=await Ee(String(n??""));return h(),a}),e.handle("device:info",()=>({deviceId:u("device_id"),branchId:u("branch_id"),apiUrl:u("api_url"),hardwareId:D(),version:c.app.getVersion()})),e.handle("device:activate",(t,n,a)=>{const[r,s]=String(n??"").split(":");if(!r||!s)throw new Error("Activation code must be <terminal-id>:<branch-id>");return N("api_url",String(a??"").replace(/\/+$/,"")),N("device_id",r.trim()),N("branch_id",s.trim()),N("hardware_id",D()),h(),{deviceId:r.trim()}})}function D(){const e=Object.values(f.networkInterfaces()).flat().filter(t=>!!t).filter(t=>!t.internal&&t.mac&&t.mac!=="00:00:00:00:00:00").map(t=>t.mac).sort();return w.createHash("sha256").update([...e,f.hostname(),f.platform()].join("|")).digest("hex").slice(0,32)}const F=A.dirname(W.fileURLToPath(typeof document>"u"?require("url").pathToFileURL(__filename).href:v&&v.tagName.toUpperCase()==="SCRIPT"&&v.src||new URL("main.js",document.baseURI).href));let o=null;function x(){o=new c.BrowserWindow({width:1440,height:900,minWidth:1024,minHeight:720,show:!1,backgroundColor:"#0b0d10",title:"DevsFleet POS",webPreferences:{preload:A.join(F,"preload.js"),contextIsolation:!0,nodeIntegration:!1,sandbox:!1,webSecurity:!0,spellcheck:!1}}),o.once("ready-to-show",()=>o?.show());const e=process.env.VITE_DEV_SERVER_URL;e?(o.loadURL(e),o.webContents.openDevTools({mode:"detach"})):o.loadFile(A.join(F,"../dist/index.html")),o.webContents.setWindowOpenHandler(()=>({action:"deny"})),o.webContents.on("will-navigate",(t,n)=>{n!==e&&t.preventDefault()}),o.on("closed",()=>{o=null})}c.app.requestSingleInstanceLock()?(c.app.on("second-instance",()=>{o&&(o.isMinimized()&&o.restore(),o.focus())}),c.app.whenReady().then(()=>{V(),ve(c.ipcMain),he(c.ipcMain,()=>o),j(c.ipcMain),x(),c.app.on("activate",()=>{c.BrowserWindow.getAllWindows().length===0&&x()})})):c.app.quit();c.app.on("window-all-closed",()=>{process.platform!=="darwin"&&c.app.quit()});c.app.on("before-quit",()=>{Se(),$()});
